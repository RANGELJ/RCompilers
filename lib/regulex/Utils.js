...
function Set (a, _sorted) {
  ...
  @return {Set} return new Set */
    a.union = function (b) {
        b = Set(b);
        const n = a.length + b.length, c = new a.constructor(n);
        for (const i = 0, j = 0, k = 0; k < n; k++) { // merge
...
function bsearch (a, x, cmp) {
    const lo = 0, n = a.length, hi = n - 1, pivot, c;
    if (n < 1) return -1;
...
function copyArray (a, size) {
    ...
    const ret = new a.constructor(size), i = size;
    while (i--) ret[i] = a[i];
    return ret;
}
...
function hashUnique (a) {
    const table = {}, i = 0, j = 0, l = a.length, x;
    for (;i < l; i++) {
        x = a[i];
...
function ____classify (ranges) {
    ...
    chars.reduce(function (prev, c) {
        const head, tail, choosed = [];
        ranges = ranges.filter(function (rg) { // side affects filter
            const start = rg[0], end = rg[1] || start;
            head = head || start == c;
            tail = tail || end == c;
        ...
        const last = stack[stack.length - 1], valid, newRange,
...
function negate (ranges /*: [Range rg] */) {
    ...
    ranges.reduce(function (acc, r) {
        const start = succ(acc[1] || acc[0]), end = pred(r[0]);
...
function toPrint (s, isRaw) {
    const ctrl = /[\x00-\x1F\x7F-\x9F]/, unicode = /[\u009F-\uFFFF]/;
