...
function Set (a, _sorted) {
  ...
  @return {Set} return new Set */
    a.union = function (b) {
        b = Set(b);
        let n = a.length + b.length, c = new a.constructor(n);
...
function copyArray (a, size) {
    ...
    let ret = new a.constructor(size), i = size;
    while (i--) ret[i] = a[i];
    return ret;
}
...
function classify (ranges) {
    ...
    let chars = sortUnique(ranges.join("").split("")),
