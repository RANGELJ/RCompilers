...
AST:
  ...
  Only Constants.AssertLookahead,Constants.AssertNegativeLookahead has `sub` property
  "(?=(abc))" repr as {
    type:"assert", assertionType:Constants.AssertLookahead,
    sub:[{
      type:"group",
...
function parse(re,_debug) {
  ...
  if (valid) {
    ...
    ast.traverse(_checkCharsetRange,Constants.CHARSET_NODE);
    
    // Check any repeats after assertion. e.g. /a(?=b)+/ doesn't make sense.
    ast.traverse(_checkRepeat,Constants.ASSERT_NODE);
...
function _filterEmptyExact(stack) {
  return stack.filter(function (node) {
    if (node.type == Constants.EXACT_NODE && node.concatTemp) {
...
function _coalesceExactNode(stack) {
  ...
  for (var i=1,j=1,l=stack.length,node;i<l;i++) {
    node=stack[i];
    if (node.type===Constants.EXACT_NODE) {
      if (prev.type===Constants.EXACT_NODE && !prev.repeat && !node.repeat) {
...
function _fixNodes(stack,re,endIndex) {
  if (!stack.length) {
    stack.push({type:Constants.EMPTY_NODE,indices:[endIndex,endIndex]});
    return;
  }
  stack.reduce(function (endIndex,node) {
    node.indices.push(endIndex);
    node.raw=re.slice(node.indices[0],endIndex);
    if (node.type===Constants.GROUP_NODE || (node.type===Constants.ASSERT_NODE && node.sub)) {
      _fixNodes(node.sub,re,node.endParenIndex);
    } else if (node.type===Constants.CHOICE_NODE) {
    ...
    } else if (node.type===Constants.EXACT_NODE) {
      if (!node.concatTemp) {
...
function _checkRepeat(node) {
  if (node.repeat) {
    ...
    if (astype === 'Constants.AssertLookahead'  || astype === 'Constants.AssertNegativeLookahead' ) {
      var assertifier = astype === 'Constants.AssertLookahead' ? '?=' : '?!';
...
var actions=(function _() {
  ...
  function exact(stack,c,i) { //any literal string.
    ...
    // node.chars = node.chars || node.raw
    var last=stack[0];
    if (!last || last.type!=Constants.EXACT_NODE || last.repeat || (last.chars && !last.concatTemp)) {
      stack.unshift({type:Constants.EXACT_NODE, indices:[i]});
    }
  ...
  function dot(stack,c,i) { //   /./
    stack.unshift({type:Constants.DOT_NODE,indices:[i]});
  }
  function nullChar(stack,c,i) {
    stack.unshift({
      type:Constants.EXACT_NODE, chars:"\0",
      indices:[i-1] 
    });
  }
  function AssertBegin(stack,c,i) { //  /^/
    stack.unshift({
      type:Constants.ASSERT_NODE,
      indices:[i],
      assertionType: Constants.AssertBegin
    });
  }
  function AssertEnd(stack,c,i,state,s) {
    stack.unshift({
      type:Constants.ASSERT_NODE,
      indices:[i],
      assertionType:Constants.AssertEnd
    });
  }
  function AssertWordBoundary(stack,c,i) {//\b \B assertion
    stack.unshift({
      type:Constants.ASSERT_NODE,
      indices:[i-1],
      assertionType: c=='b'?Constants.AssertWordBoundary:Constants.AssertNonWordBoundary
    });
  }
  function repeatnStart(stack,c,i) { //  /a{/
    ...
    //Because /a{+/ is valid.
    var last=stack[0];
    if (last.type===Constants.EXACT_NODE) {
      return;
    } else { // '[a-z]{' is valid
      stack.unshift({type:Constants.EXACT_NODE,indices:[i]});
    }
  ...
  function _repeat(stack,min,max,charEndIndex,s) {
    ...
    if (last.type===Constants.EXACT_NODE) { // exact node only repeat last char
      var a={
        type:Constants.EXACT_NODE,
  ...
  function escapeStart(stack,c,i) {
    stack.unshift({
      concatTemp:true,
      type:Constants.EXACT_NODE,chars:"",indices:[i]
    });
  }
  function normalEscape(stack,c,i) {
    ...
    stack.unshift({
      type:Constants.EXACT_NODE,chars:c,indices:[i-1]
    });
  }
  function charClassEscape(stack,c,i) {

    stack.unshift({
      type:Constants.CHARSET_NODE,indices:[i-1],chars:'',ranges:[],
  ...
  function hexEscape(stack,c,i,state,s) {
    ...
    stack.unshift({
      type:Constants.EXACT_NODE, chars:c,
  ...
  function groupToAssertion(stack,c,i) { // Convert /(?!)/,/(?=)/ to AssertNode
    var group=stack._parentGroup;
    group.type=Constants.ASSERT_NODE;
    group.assertionType= c=='=' ? Constants.AssertLookahead : Constants.AssertNegativeLookahead ;
  ...
  function choice(stack,c,i) { // encounters "|"
    ...
    if (stack._parentChoice) {
      ...
      choice={
        type:Constants.CHOICE_NODE,indices:[(first?first.indices[0]:i-1)],
        branches:[]
      };
  ...
  function charsetStart(stack,c,i) {
    stack.unshift({
      type:Constants.CHARSET_NODE,indices:[i],
  ...
  */
  function backref(stack,c,i,state) {
    ...
    if (!isFirstNum) { //previous node must be backref node
      ...
      last={type:Constants.BACKREF_NODE,indices:[i-1]};
      stack.unshift(last);
    }
    if (n>cn) {
      throw new RegexSyntaxError({
  ...
  return {
    ...
    exact:exact,dot:dot,nullChar:nullChar,AssertBegin:Constants.AssertBegin,
    AssertEnd:Constants.AssertEnd,AssertWordBoundary:Constants.AssertWordBoundary,
